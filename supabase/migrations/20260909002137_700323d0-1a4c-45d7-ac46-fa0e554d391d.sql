-- Renumber the approved working sequence to consecutive ordinals so the
-- "one step at a time" validation can compare them directly. Legacy rows
-- stay at ordinals 900+.
with ordered as (
  select milestone_key,
         row_number() over (order by ordinal, milestone_key) - 1 as new_ordinal
    from public.milestone_catalog
   where ordinal < 900
)
update public.milestone_catalog c
   set ordinal = o.new_ordinal
  from ordered o
 where o.milestone_key = c.milestone_key
   and c.ordinal <> o.new_ordinal;

-- Compare working milestones by their position in the approved sequence.
...
begin
  ...
  -- A legacy current value (kept for history) is always allowed to advance
  -- once onto the approved sequence.
